...
namespace ORB_SLAM2 {
    ...
    dynamic_detection_confidence_threshold(dynamic_detection_confidence_threshold_)
    {
        ...
        detect_net_ptr->opt.use_vulkan_compute = false;
    ...
    void Detector2D::Run() {
        ...
        while(1)
        {
           ...
           // auto start = std::chrono::high_resolution_clock::now();
            ...
            // 计算检测时间
            //std::chrono::duration<double, std::milli> detection_time = end - start;
            //std::cout << "Detection time: " << detection_time.count() << " ms" << std::endl;
            ImageDetectFinished();
        }
    }
