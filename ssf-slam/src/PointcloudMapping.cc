...
global_pc_update_kf_threshold(global_pc_update_kf_threshold_)
{
    ...
    if(is_global_pc_reconstruction)
    {
        ...
        //std::cout << "Original Point Cloud Size: " << globalMap->points.size() << std::endl;
        ...
        //std::cout << "After Voxel Grid Filtering: " << cloud_voxel_filtered->points.size() << std::endl;
...
void PointCloudMapping::MapViewer()
{
    ...
    if(is_octo_semantic_map_construction)
    {
        local_pcl_publisher = nh.advertise<sensor_msgs::PointCloud2>("/SSF_SLAM/Local_Point_Clouds",10);
        marker_publisher= nh.advertise<visualization_msgs::Marker>("/SSF_SLAM/Semantic_Objects",10);
        settingTextMarkerBasicParameter(0.2);
        settingCubeMarkerBasicParameter();
    }
    if(is_global_pc_reconstruction) 
        global_pcl_publisher = nh.advertise<sensor_msgs::PointCloud2>("/SSF_SLAM/Global_Point_Clouds",10);
    ...
    while(1)
    {
        ...
        {
            static int count = 0;
            //std::cout << "Number of points in global map before voxel filtering: " << globalMap->points.size() << std::endl;
            voxel_global.setInputCloud(temp_globalMap);
            voxel_global.filter(*temp_globalMap);
            //std::cout << "Number of points after voxel filtering: " << temp_globalMap->points.size() << std::endl;
            sor_global.setInputCloud(temp_globalMap);
            sor_global.filter(*temp_globalMap); //ros mode
            //std::cout << "Number of points after outlier removal: " << temp_globalMap->points.size() << std::endl;
            *globalMap += *temp_globalMap;
            ...
            if(!CheckNewKeyFrames() || (count > global_pc_update_kf_threshold))
            {
                ...
                //std::cout << "Number of points in global map after voxel filtering: " << globalMap->points.size() << std::endl;
                sor_global.setInputCloud(globalMap);
                sor_global.filter(*globalMap); //ros mode
                //std::cout << "Number of points in global map after outlier removal: " << globalMap->points.size() << std::endl;
