...
cv::Mat Camera_Pose;
tf::Transform ssf_slam_tf;
tf::TransformBroadcaster *ssf_slam_tf_broadcaster;
...
{
    ros::init(argc, argv, "ssf_slam_ros_rgbd");
    ros::start();

    if(argc != 3)
    {
        cerr << endl << "Usage: rosrun ssf_slam_ros_rgbd path_to_vocabulary path_to_settings" << endl;        
        ros::shutdown();
        return 1;
    }
...
void Pub_CamPose(cv::Mat &pose)
{
	...
	ssf_slam_tf_broadcaster = new tf::TransformBroadcaster;
	...
	else
	{
		...
		ssf_slam_tf.setOrigin(tf::Vector3(twc.at<float>(2), -twc.at<float>(0), -twc.at<float>(1)));
		ssf_slam_tf.setRotation(tf::Quaternion(Q.z(), -Q.x(), -Q.y(), Q.w()));
		//ssf_slam_tf_broadcaster.sendTransform(tf::StampedTransform(ssf_slam_tf, ros::Time::now(), "/map", "/camera"));
		//delete ssf_slam_tf_broadcaster;
		Cam_Pose.header.stamp = ros::Time::now();
		Cam_Pose.header.frame_id = "/map";
		tf::pointTFToMsg(ssf_slam_tf.getOrigin(), Cam_Pose.pose.position);
		tf::quaternionTFToMsg(ssf_slam_tf.getRotation(), Cam_Pose.pose.orientation);
		...
		tf::pointTFToMsg(ssf_slam_tf.getOrigin(), Cam_odom.pose.pose.position);
		tf::quaternionTFToMsg(ssf_slam_tf.getRotation(), Cam_odom.pose.pose.orientation);
